...
function Blog(props){
  return (
    <article className='w-[60%] p-5 m-5 rounded-md flex flex-row justify-between bg-beige'>
      <img src={props.image.src} alt={props.image.alt} className='w-[25%] h-auto rounded-md'/>
      <div className='p-5'>
        <div className='flex flex-row items-center gap-10'>
          ...
          <a href={props.googleMapsLink} className='text-smokyBlack underline'>View on Google Maps</a>
        </div>
        <div className='text-left text-smokyBlack mt-5'>
          <h2 className='text-3xl font-bold mb-2'>{props.title}</h2>
          <h3 className='text-md font-bold'>{props.dates}</h3>
          <p>{props.text}</p>
        </div>
      </div>
    </article>   
